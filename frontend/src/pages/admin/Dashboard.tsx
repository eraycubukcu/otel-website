...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [dashboardRes, visitorRes] = await Promise.all([
          dashboardService.getStats(),
          visitorService.getStats(),
        ]);

        setData(dashboardRes);
        setVisitorData(visitorRes); // ziyaterçi verisi set
      } catch (error) {
        console.error(error);
  ...
  const stats = [
    ...
    },
    {
      ...
      desc: `${data.stats?.totalRooms || 0} odadan ${
        data.stats?.occupiedRooms || 0
      }'i dolu`,
    },
    {
      title: "Toplam Ziyaretçi",
      value: visitorData.totalVisitors.toLocaleString("tr-TR"),
      icon: <Globe className="h-5 w-5 text-teal-600" />,
      desc: "Siteye giren tekil kişi sayısı",
    },
    {
      title: "Bugünkü Ziyaretçi",
      value: visitorData.todayVisitors,
      icon: <ActivityIcon className="h-5 w-5 text-rose-600" />,
      desc: "Son 24 saatteki ziyaretler",
    },
  ];
        ...
        {stats.map((stat, index) => (
          ...
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <div className="p-2 bg-slate-50 rounded-full">{stat.icon}</div>
            </CardHeader>
            <CardContent>
